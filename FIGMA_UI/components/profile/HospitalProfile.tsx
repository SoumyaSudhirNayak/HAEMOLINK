...
import { User, Mail, Phone, MapPin, Building2, Shield, Bell, Settings, ChevronRight, LogOut, CheckCircle, ArrowLeft, Award, Heart } from 'lucide-react';
...
export function HospitalProfile({ onBack, onLogout }: HospitalProfileProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const loc = (profile?.address || '').split(',');
        const adminContact = String(profile?.admin_contact || '');
        const adminContactTokens = adminContact.split(' ').filter(Boolean);
        const maybePhone = adminContactTokens[adminContactTokens.length - 1] || '';
        const phone = /[0-9]/.test(maybePhone) ? maybePhone : '';
        setUserData({
          name: profile?.organization_name || user?.email || 'Institution',
  ...
  const profileOptions = [
    {
      icon: User,
      label: 'Edit Profile',
      description: 'Update your institutional information',
      onClick: () =>
        toast('Edit Profile', {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-violet-50 to-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl text-gray-900 mb-2">Institution Profile</h1>
          <p className="text-gray-600">Manage your hospital account and preferences</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8 mb-6">
          <div className="flex items-start gap-4 md:gap-6">
            {/* Avatar */}
            <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-violet-500 to-violet-600 rounded-2xl flex items-center justify-center flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="inline-flex items-center px-3 py-1 bg-violet-100 text-violet-700 rounded-lg text-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                ...
                <div className="flex items-center gap-3 text-gray-600">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm">{userData.licenseNumber}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm">{userData.city}, {userData.state}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex flex-wrap gap-3">
                <div className="inline-flex items-center gap-2 px-3 py-2 bg-violet-50 text-violet-700 rounded-lg text-sm">
                  ...
                  <span><strong>{userData.totalRequests}</strong> requests fulfilled</span>
                </div>
                <div className="inline-flex items-center gap-2 px-3 py-2 bg-green-50 text-green-700 rounded-lg text-sm">
                  <CheckCircle className="w-4 h-4" />
                  <span><strong>{userData.successRate}%</strong> success rate</span>
                </div>
              </div>
            </div>
          </div>
